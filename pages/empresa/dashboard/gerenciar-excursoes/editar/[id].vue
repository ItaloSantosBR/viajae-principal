  ...
  <div>
    <NuxtLayout name="dashboard">
      ...
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Editar Excursão</h1>
        <NuxtLink :to="`/empresa/dashboard/gerenciar-excursoes/${excursionId}`" class="btn btn-outline flex items-center">
      ...
      <div v-else>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
          <div class="flex flex-wrap justify-between mb-8">
            ...
            >
              <div 
                class="w-10 h-10 rounded-full flex items-center justify-center mb-2 border-2 cursor-pointer"
                :class="[
          ...
          <div>
            ...
            <div v-if="currentStep === 7">
              ...
              <div class="space-y-6">
                  ...
                  <div class="flex items-start">
                    ...
                    <div>
                      ...
                      <p class="text-yellow-600">
                        Verifique todas as informações antes de salvar. Algumas informações não poderão ser alteradas caso já existam reservas.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <!-- Validation summary for final step -->
                <div v-if="Object.keys(allStepErrors).length > 0" class="bg-red-50 p-4 rounded-lg border border-red-200">
                  <h3 class="font-bold text-red-700 mb-2">Existem campos obrigatórios não preenchidos:</h3>
                  <ul class="list-disc pl-5 text-red-600">
                    <li v-for="(stepErrors, stepIndex) in allStepErrors" :key="stepIndex">
                      {{ steps[stepIndex].name }}:
                      <ul class="list-disc pl-5">
                        <li v-for="(error, field) in stepErrors" :key="field">{{ error }}</li>
                      </ul>
                    </li>
                  </ul>
                  <p class="mt-2 text-red-700">Por favor, preencha todos os campos obrigatórios antes de salvar as alterações.</p>
                </div>
              </div>
            </div>
          </div>
...
const isFormValid = computed(() => {
  validateAllSteps();
  return Object.keys(allStepErrors).length === 0;
});
...
const validateCurrentStep = () => {
  ...
  switch (currentStep.value) {
    ...
    case 7: // Revisão e Publicação
      if (!formData.termsAccepted) {
        errors.termsAccepted = 'Você precisa aceitar os termos para salvar as alterações';
      }
      break;
  }
  ...
  if (Object.keys(errors).length > 0) {
    allStepErrors[currentStep.value] = { ...errors };
  } else {
    delete allStepErrors[currentStep.value];
  }
...
const saveExcursion = async () => {
  ...
  if (!validateAllSteps()) {
    alert('Por favor, preencha todos os campos obrigatórios antes de salvar as alterações.');
    return;
  }
  
  try {
    // In a real app, this would save to the database
    const success = await excursionsStore.updateExcursion(excursionId, {
    ...
    if (success) {
      ...
      router.push('/empresa/dashboard/gerenciar-excursoes');
    } else {
      alert('Erro ao atualizar excursão. Por favor, tente novamente.');
    }
  } catch (error) {
...
onMounted(async () => {
  ...
  try {
    ...
    const excursion = excursionsStore.getExcursionById(excursionId);
    
    if (!excursion) {
      error.value = 'Excursão não encontrada.';
      return;
    }
    
    // Populate form data with excursion details
    formData.title = excursion.title;
    formData.description = excursion.description;
    formData.category = excursion.category;
    formData.images = [...excursion.images];
    formData.originCity = excursion.originCity;
    formData.destination = excursion.destination;
    formData.startDate = excursion.startDate;
    formData.endDate = excursion.endDate;
    formData.capacity = excursion.capacity;
    formData.availableSpots = excursion.availableSpots;
    formData.accommodation = { ...excursion.accommodation };
    formData.includedServices = [...excursion.includedServices];
    formData.price = excursion.price;
    
  } catch (err) {
    error.value = err.message || 'Ocorreu um erro ao carregar os dados da excursão.';
  } finally {
    loading.value = false;
  }
});
...
This completes the Edit Excursion page implementation. The page reuses most of the components and logic from the Create Excursion page but pre-populates the form with the existing excursion data and handles updates instead of creation.

The key differences from the Create page are:

1. Different page title and back button
2. Loading existing excursion data on mount
3. Different submit action (update instead of create)
4. Warning about changes that can't be made if there are existing reservations

The page maintains all the validation, step navigation, and form organization from the Create page while adapting it for edit
